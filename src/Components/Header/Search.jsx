...
const Search = ({
  ...
  const [open, toggleOpen] = useState(suggestions.length > 1);
  const [openCriteria, toggleCriteria] = useState(false);
  const [filter, changeFilter] = useState(
    'Cliquer ici pour changer le filter... ',
  );
					...
					<MenuList>
						{suggestions.map(suggetion => (
							<MenuItem
								...
								onClick={() => handleRequest(suggetion)}
								style={{ height: '15px' }}
							>
								{suggetion}
							</MenuItem>
					...
					>
						<Paper>
							<MenuList>
								...
								<MenuItem onClick={() => changeSearchFilter('SKILLS')}>
									par compétence
								</MenuItem>
							</MenuList>
						</Paper>
...
export default compose(
  ...
  connect(
    mapStateToProps,
    { changeSearchKeyWord, changeSearchCriteria },
  ),
)(Search);
