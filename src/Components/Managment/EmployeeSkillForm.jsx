...
const EmployeeSkillForm = ({
  ...
  const skillsItems = skillsSuggestion.map(({ _id, name }) => (
  ...
  return (
		<Formik initialValues={{ skill: '', level: '', expYears: '' }}>
			{({ handleChange, values: { skill, level, expYears } }) => (
				<TableRow className={className}>
					...
					<TableCell>
						<Button
							...
							color="primary"
							onClick={() => {
							  if (contains(skill, skillIds)) {
							    openModal({
							      title: 'Compétences',
							      body: <h4>Compétences existe deja</h4>,
							    });
							  } else {
							    addSkill({ skill, level, expYears });
							  }
							}}
						>
							{' '}
...
export default compose(
  ...
  connect(
    mapStateToProps,
    { addSkill, getAllSkillSuggestion, openModal },
  ),
)(EmployeeSkillForm);
